...
const getOppositeLightness = (hex) => {
  ...
  if (!/^([0-9a-f]{3}|[0-9a-f]{6})$/.test(hex)) {
    throw new Error(
      "Invalid hex color format. Color value must be a hex without #"
    );
  }
...
const handleImageGeneration = (req, res) => {
  ...
  if (!resolution) {
    throw new Error(
      "Missing image dimensions. Expected /image/[width]x[height]"
    );
  }
  ...
  if (!width || !height || isNaN(width) || isNaN(height)) {
    throw new Error(
      "Invalid image dimensions. Expected 'width' and 'height' to be numeric."
    );
  }
...
server.listen(PORT, () => {
  console.log(`Placeholdy server is running on port ${PORT}`);
});
